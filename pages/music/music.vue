		...
		<view class="radius-lg">
			<view class="bg-white padding-bottom radius-xl" style="text-align: center;">
				...
				<view class="padding-lr margin-xxl" style="height: 15rem; margin-bottom: 40px">
					<image class="shadow-blur bg-img radius-lg" style="" :src="audioData[now].song_image"></image>
				</view>
				...
				<view class="text-center padding-top-sm">
					<text class="text-sm text-gray text-center" style="font-size:20px">{{audioData[now].singername}}</text>
				</view>
				<view class="margin-top-xl text-center text-xxxl text-black padding-top-lg">
					...
					<a-button type="link" ghost @click="like_ornot(now)">
						<text v-if="audioData[now].like==1" class="cuIcon-likefill text-red padding-lr-xl iconbtn" alt></text>
				...
				<view class="content">
					<imt-audio autoplay="true" continue="true" :playWay="playWay" :src="audioData[now].file" :duration="audioData[now].file.duration"
						@prev="now = now === 0?audioData.length-1:now-1"
